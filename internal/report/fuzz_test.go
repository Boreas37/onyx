...
// fields of a Result (including remediation guidance and patched versions)
// and asserts the HTML, CSV, Markdown, JUnit and CycloneDX writers never
// panic, never emit a literal <script> tag, and never produce a CSV cell
	...
	f.Fuzz(func(t *testing.T, rating, title, slug, remediation string) {
		...
		res := &scanner.Result{
			...
			Findings: []scanner.Finding{{
				...
				Vulnerabilities: []scanner.Vulnerability{v},
			}},
		}
